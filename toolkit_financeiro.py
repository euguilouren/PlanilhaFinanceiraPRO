...
def validar_config(cfg: dict) -> list[str]:
    ...
    for chave in ("liquidez_corrente_min", "liquidez_seca_min", "margem_liquida_min", "endividamento_max", "roe_min"):
        val = ind.get(chave)
...
class Leitor:
    ...
    def ler_arquivo(caminho: str, **kwargs) -> dict:
        ...
        for aba_info in diagnostico["abas"]:
            ...
            diagnostico["problemas_formato"].extend(Leitor._detectar_problemas_formato(df, aba_info["nome"]))
    ...
    def _detectar_problemas_formato(df: pd.DataFrame, aba: str) -> list:
        ...
        for col in df.columns:
            if df[col].dtype == "object":
                numeros_texto = (
                    df[col]
                    .apply(lambda x: bool(Leitor._RE_NUMERO.match(str(x).strip())) if pd.notna(x) else False)
                    .sum()
                )
                ...
                formatos = [
                    nome_fmt
                    for nome_fmt, padrao in padroes_data.items()
                    if df[col].apply(lambda x: bool(re.match(padrao, str(x).strip())) if pd.notna(x) else False).sum()
                    > 0
                ]
                if len(formatos) > 1:
                    problemas.append(
...
class Auditor:
    ...
    @staticmethod
    def detectar_inconsistencias_temporais(
        df: pd.DataFrame, col_data: str, col_data2: str = None, aba: str = ""
    ) -> list:
        inconsistencias = []
        ...
        for col in colunas_obrigatorias:
            ...
            if len(vazios) > 0:
                inconsistencias.append(
                    {
                        "aba": aba,
                        "linha": (vazios.index + 2).tolist()[:10],
                        "coluna": col,
                        "tipo": "CAMPO_VAZIO",
    ...
    def detectar_classificacao_errada(df: pd.DataFrame, col_valor: str, col_tipo: str, aba: str = "") -> list:
        ...
        receitas_neg = df[df[col_tipo].str.upper().str.contains("RECEITA|VENDA|FATURAMENTO", na=False) & (valores < 0)]
...
class Conciliador:
    ...
    def conciliar_aproximado(
        ...
        d1 = (
            pd.to_datetime(df1[col_data1], errors="coerce", dayfirst=True)
            if col_data1 and col_data1 in df1.columns
            else pd.Series([pd.NaT] * len(df1))
        )
        d2 = (
            pd.to_datetime(df2[col_data2], errors="coerce", dayfirst=True)
            if col_data2 and col_data2 in df2.columns
            else pd.Series([pd.NaT] * len(df2))
        )
                ...
                matches.append(
                    {
                        ...
                        "Status": (
                            f"{Status.OK} (aprox.)"
                            if abs(diff_r) <= tolerancia_valor
                            else f"{Status.DIVERGENTE} (aprox.)"
                        ),
                ...
                matches.append(
                    {
                        ...
                        f"Linha_{nome_fonte2}": "",
                        f"Valor_{nome_fonte2}": "",
                        "Diferença_R$": "",
                        "Score_Match": 0,
                        "Status": f"NÃO ENCONTRADO em {nome_fonte2}",
                        "Confiança": "",
                    }
                )
...
class AnalistaFinanceiro:
    ...
    def calcular_aging(
        ...
        def _faixa(d):
            ...
            if d < 0:
                return "A vencer"
            if d <= faixa_atencao:
                return "Vencido 1-30 dias"
            if d <= faixa_media:
                return "Vencido 31-60 dias"
            if d <= faixa_critica:
                return "Vencido 61-90 dias"
            return "Vencido +90 dias"

        df["Faixa_Aging"] = dias.apply(_faixa)
        df["Dias_Atraso"] = dias.clip(lower=0)
        resumo = (
            df.groupby("Faixa_Aging")
            .agg(
        ...
        mapa_dre = OrderedDict(
            [
                ...
                (
                    ...
                    ["DESPESA ADMINISTRATIVA", "DESPESA COMERCIAL", "DESPESA OPERACIONAL", "DESPESA GERAL"],
                ),
                (
                    "(-/+) Resultado Financeiro",
                    ["RECEITA FINANCEIRA", "DESPESA FINANCEIRA", "JUROS", "VARIAÇÃO CAMBIAL"],
                ),
        ...
        for idx, row in grupo.iterrows():
            ...
            for i, col in enumerate(cols):
                ...
                if i > 0:
                    ...
                    r[f"Var_{cols[i-1]}→{col}_%"] = (
                        round((atual - anterior) / anterior * 100, 1) if anterior != 0 else 0
                    )
    ...
    def classificar_impostos_br(df: pd.DataFrame, col_categoria: str) -> pd.DataFrame:
        ...
        def _classif(c):
            for x in DEDUCAO:
                if x in c:
                    return "Dedução de Receita"
            for x in RESULTADO:
                if x in c:
                    return "IR/CSLL (após resultado)"
            for x in ENCARGOS:
                if x in c:
                    return "Despesa Operacional (encargos)"
    ...
    def indicadores_saude(
        ...
        """
        th = {"lc_min": 1.0, "ls_min": 0.8, "li_min": 0.3, "ml_min": 5.0, "end_max": 100.0, "roe_min": 15.0}
        if thresholds:
        ...
        if passivo_circulante != 0:
            ...
            _add(
                "Liquidez Corrente", "Ativo Circ. / Passivo Circ.", lc, f'> {th["lc_min"]}', lambda v: v >= th["lc_min"]
            )
            if estoque > 0:
                ls = round((ativo_circulante - estoque) / passivo_circulante, 2)
                _add(
                    "Liquidez Seca",
                    "(Ativo Circ. - Estoque) / Passivo Circ.",
                    ls,
                    f'> {th["ls_min"]}',
                    lambda v: v >= th["ls_min"],
                )
            if caixa > 0:
        ...
        indicadores.append(
            {
                "Indicador": "Capital de Giro (NCG)",
                "Fórmula": "Ativo Circ. - Passivo Circ.",
                "Valor": cg,
                "Referência": "> 0",
                "Status": "SAUDÁVEL" if cg > 0 else "CRÍTICO",
            }
        )
        ...
        if receita_liquida > 0 and lucro_liquido != 0:
            ...
            _add(
                "Margem Líquida (%)",
                "Lucro Líq. / Receita Líq. × 100",
                ml,
                f'> {th["ml_min"]}%',
                lambda v: v >= th["ml_min"],
            )

        if patrimonio_liquido > 0 and divida_total > 0:
            end = round(divida_total / patrimonio_liquido * 100, 1)
            _add(
                "Endividamento (%)", "Dívida Total / PL × 100", end, f'< {th["end_max"]}%', lambda v: v < th["end_max"]
            )
    ...
    def resumo_periodo(
        ...
        if col_tipo not in df.columns:
            ...
            df["_tipo"] = _tipo.where(
                _tipo.isin(["RECEITA", "DESPESA"]),
                other=df["_valor"].apply(lambda v: "RECEITA" if v >= 0 else "DESPESA"),
            )
        ...
        combined["Resultado_Pct"] = combined.apply(
            lambda r: round(r["Resultado_RS"] / r["Receita_RS"] * 100, 1) if r["Receita_RS"] != 0 else 0.0, axis=1
        )
        combined["Receita_RS"] = combined["Receita_RS"].round(2)
        combined["Despesa_RS"] = combined["Despesa_RS"].round(2)

        return combined[
            ["Periodo", "Receita_RS", "NFs_Receita", "Despesa_RS", "NFs_Despesa", "Resultado_RS", "Resultado_Pct"]
        ]
...
class AnalistaComercial:
    ...
    def pareto(df: pd.DataFrame, col_entidade: str, col_valor: str, top_pct: float = 0.8) -> pd.DataFrame:
        agrupado = (
            df.groupby(col_entidade)
            .agg(Total_RS=(col_valor, lambda x: pd.to_numeric(x, errors="coerce").sum()))
            .reset_index()
    ...
    def realizado_vs_meta(
        ...
        merged = pd.merge(
            df_realizado.groupby(col_chave)
            .agg(Realizado_RS=(col_valor_real, lambda x: pd.to_numeric(x, errors="coerce").sum()))
            .reset_index(),
...
class Util:
    ...
    def gerar_id_registro(df: pd.DataFrame, colunas: list) -> pd.Series:
        ...
        return (
            df[colunas].astype(str).agg("|".join, axis=1).apply(lambda x: hashlib.sha256(x.encode()).hexdigest()[:12])
        )
    ...
    def detectar_entidades_similares(series: pd.Series, threshold: float = 0.8) -> list:
        ...
        for i, n1 in enumerate(nomes_upper):
            ...
            if len(grupo) > 1:
                visitados.add(i)
                contagens = {
                    n: int((series.astype(str).str.upper().str.strip() == str(n).upper().strip()).sum()) for n in grupo
                }
...
class PrestadorContas:
    ...
    def demonstrativo_movimentacao(
        ...
        linhas = [
            {
                "Descrição": f"SALDO INICIAL ({periodo})",
                "Valor": round(saldo_inicial, 2),
                "Natureza": "",
                "Tipo": "SALDO",
            },
        ...
        for _, row in cat_ent.sort_values("Total", ascending=False).iterrows():
            linhas.append(
                {
                    "Descrição": f"  {row['Categoria']}",
                    "Valor": round(row["Total"], 2),
                    "Natureza": f"{int(row['Qtd'])} lançamentos",
                    "Tipo": "DETALHE",
                }
            )
        linhas += [
    ...
    def orcado_vs_realizado(
        ...
        col_categoria: str,
        col_valor_real: str,
        col_valor_orcado: str,
    ) -> pd.DataFrame:
        real = (
            df_realizado.groupby(col_categoria)
            .agg(Realizado=(col_valor_real, lambda x: pd.to_numeric(x, errors="coerce").sum()))
            .reset_index()
        )
        orc = (
            df_orcado.groupby(col_categoria)
            .agg(Orçado=(col_valor_orcado, lambda x: pd.to_numeric(x, errors="coerce").sum()))
            .reset_index()
        )
        ...
        linhas.append(
            {
                ...
                "Variação_%": (
                    round((total_final - total_inicial) / total_inicial * 100, 1) if total_inicial != 0 else 0
                ),
            }
        )
        return pd.DataFrame(linhas)
...
class MontadorPlanilha:
    ...
    @staticmethod
    def _safe_value(valor) -> str | float | int | None:
    ...
    def adicionar_aba(
        ...
        titulo: str = None,
        col_status: str = None,
        cols_moeda: list = None,
        cols_pct: list = None,
        cols_data: list = None,
        congelar: str = None,
        adicionar_totais: bool = True,
        cols_soma: list = None,
        cols_contagem: list = None,
    ) -> None:
        ...
        for col_name, col_idx in col_map.items():
            ...
            cell.border = Estilos.BORDER_HEADER
        ...
        if adicionar_totais and len(df) > 0:
            ...
            _borda = Border(
                top=Side("double", "1F4E79"),
                bottom=Side("double", "1F4E79"),
                left=Side("thin", "B0B0B0"),
                right=Side("thin", "B0B0B0"),
            )
            _fill = PatternFill("solid", fgColor="D6E4F0")
            _font = Font(name="Arial", bold=True, size=10, color="1F4E79")
            ws.cell(row=totais_row, column=1, value="TOTAIS").font = Font(
    ...
    def adicionar_formula_coluna(
        self, nome_aba: str, col_destino: int, header: str, formula_template: str, number_format: str = None
    ) -> None:
        nome_aba = nome_aba[:31]
        if nome_aba not in self._aba_meta:
            return
        meta = self._aba_meta[nome_aba]
        ws = self.wb[nome_aba]
        hcell = ws.cell(row=meta["start_row"], column=col_destino, value=header)
        hcell.font = Estilos.FONT_HEADER
        hcell.fill = Estilos.FILL_HEADER
        hcell.alignment = Estilos.ALIGN_CENTER
        hcell.border = Estilos.BORDER_HEADER
    ...
    def gerar_mapa_formulas(self, formulas: list) -> None:
        ...
        ws.cell(row=2, column=1, value="Documentação para rastreabilidade e auditoria.").font = Font(
    ...
    def adicionar_resumo_executivo(self, metricas: dict) -> None:
        ...
        t.alignment = Alignment(horizontal="center")
        ws.cell(row=2, column=1, value=f'Gerado em: {datetime.now().strftime("%d/%m/%Y %H:%M")}').font = Font(
            name="Arial", size=9, color="808080"
        )
        row = 4
        ...
        for nome, info in metricas.items():
            ...
            if tipo == "moeda":
                vc.number_format = Estilos.FMT_MOEDA
            elif tipo == "pct":
                vc.number_format = Estilos.FMT_PERCENTUAL
...
class Verificador:
    ...
    def verificar_formulas_planilha(caminho_xlsx: str) -> dict:
        ...
        for ws_name in wb.sheetnames:
            ws = wb[ws_name]
            aba_info = {"nome": ws_name, "total_celulas": 0, "celulas_com_formula": 0, "celulas_com_valor_numerico": 0}
            for row in ws.iter_rows():
                for cell in row:
                    ...
                    if isinstance(cell.value, str) and cell.value.upper().strip() in (
                        "TOTAIS",
                        "TOTAL",
                        "TOTAL GERAL",
                        "SOMA",
    ...
    def verificar_atualizacao(
        ...
        resultado.update(
            {
                "original": {"registros": len(df_original), "soma": round(soma_o, 2)},
        ...
        if abs(soma_r - esperado) > 0.01:
            resultado["alertas"].append(
                {
                    "tipo": "SOMA_ATUALIZACAO_DIVERGENTE",
                    "severidade": Status.CRITICA,
        ...
        if chaves_existentes:
            ...
            if len(dups) > 0:
                resultado["alertas"].append(
                    {
                        "tipo": "DUPLICATAS_POS_ATUALIZACAO",
                        "severidade": Status.ALTA,
...
class PipelineFinanceiro:
    ...
    def executar_conciliacao(
        ...
        logger.info(
            "Conciliação: %.1f%% OK | divergências: R$ %,.2f", resumo["percentual_ok"], resumo["soma_divergencias_rs"]
        )
        return df_concil
...
class Normalizador:
    ...
    COLUNAS_PADRAO: list[dict] = [
        ...
        {"nome": "Data", "tipo": "data", "obrigatorio": True, "descricao": "Data de emissão (DD/MM/AAAA)"},
        {"nome": "Vencimento", "tipo": "data", "obrigatorio": True, "descricao": "Data de vencimento (DD/MM/AAAA)"},
        {
            "nome": "Valor",
            "tipo": "moeda",
            "obrigatorio": True,
        ...
        },
        {
            ...
            "tipo": "lista",
            "obrigatorio": True,
            "opcoes": ["RECEITA", "CMV", "DESPESA OPERACIONAL", "DESPESA FINANCEIRA", "IMPOSTO", "OUTRO"],
        ...
        {"nome": "Cliente", "tipo": "texto", "obrigatorio": True, "descricao": "Nome do cliente ou fornecedor"},
        {
            "nome": "Status",
            "tipo": "lista",
            "obrigatorio": False,
        ...
        {"nome": "Observacao", "tipo": "texto", "obrigatorio": False, "descricao": "Observação livre (opcional)"},
    ]
        ...
        if "Tipo" in df_norm.columns:
            ...
            if sem_tipo.any():
                ...
                tipo_serie = tipo_serie.where(
                    ~(tipo_serie.str.contains(_SAIDA_VALS, na=False, regex=True) & sem_tipo), other="RECEITA"
                )
                tipo_serie = tipo_serie.where(
                    ~(tipo_serie.str.contains(_ENTRADA_VALS, na=False, regex=True) & sem_tipo), other="DESPESA"
                )
                sem_tipo = ~tipo_serie.isin(["RECEITA", "DESPESA"])
            ...
            if sem_tipo.any() and "Categoria" in df_norm.columns:
                ...
                tipo_serie = tipo_serie.where(
                    ~(cat.str.contains(_REC_CATS, na=False, regex=True) & sem_tipo), other="RECEITA"
                )
                tipo_serie = tipo_serie.where(
                    ~(cat.str.contains(_DESP_CATS, na=False, regex=True) & sem_tipo), other="DESPESA"
                )
                sem_tipo = ~tipo_serie.isin(["RECEITA", "DESPESA"])
            ...
            if sem_tipo.any() and "Valor" in df_norm.columns:
                ...
                tipo_serie = tipo_serie.where(
                    ~(sem_tipo), other=valores.apply(lambda v: "RECEITA" if v >= 0 else "DESPESA")
                )
    ...
    def validar(df: pd.DataFrame) -> list[dict]:
        ...
        for col_def in Normalizador.COLUNAS_PADRAO:
            ...
            if col_def["obrigatorio"]:
                if col_def["tipo"] == "moeda":
                    vazios = pd.to_numeric(serie, errors="coerce").isna() | (pd.to_numeric(serie, errors="coerce") == 0)
                else:
            ...
            if col_def["tipo"] == "lista" and col_def.get("opcoes"):
                ...
                invalidos_mask = ~serie.astype(str).str.upper().isin(opcoes + ["", "PENDENTE"])
            ...
            if col_def["tipo"] == "data":
                ...
                invalidas = serie.astype(str).apply(lambda x: bool(x) and x not in ("", "nan") and not re_data.match(x))
                    ...
                    problemas.append(
                        {
                            ...
                            "severidade": Status.ALTA,
                            "descricao": (
                                f"'{nome}' com formato inválido em {len(linhas_data)} linha(s). " "Use DD/MM/AAAA."
                            ),
                        }
                    )
                ...
                problemas.append(
                    {
                        ...
                        "descricao": (
                            ...
                            f"{', '.join(str(n) for n in dup_nfs[:5])}" + (" ..." if len(dup_nfs) > 5 else "")
                        ),
                    }
                )
        ...
        n_ok = len(df) - sum(
            len(p["linha"]) if isinstance(p["linha"], list) else (1 if p["linha"] else 0) for p in problemas
        )
    ...
    def gerar_template(caminho: str = "template_padrao.xlsx") -> str:
        ...
        ws_dados.cell(
            dica_row, 1, "⬆ Apague as linhas de exemplo acima e insira seus dados a partir da linha 2."
        ).font = Font(name="Arial", italic=True, size=9, color="9BA8B5")
        ws_dados.merge_cells(start_row=dica_row, start_column=1, end_row=dica_row, end_column=len(headers))
